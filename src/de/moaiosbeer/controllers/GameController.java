...
//			rd.forward(request, response);
		...
		if(mode==null)
		{
			mode = (String) request.getAttribute(RequestConstants.ATTR_REQUESTMODE);
			if(mode==null)
			{
...
//		        	Response.status(resp.getStatus()).build();
		...
		try
		{
			gameID = Long.parseLong(request.getParameter(RequestConstants.ATTR_GAMEID));
		}
		catch(Exception e)
		{
...
//						Response.status(resp.getStatus()).build();
						...
						g.updateGame(games.get(gameID),userid);
						writelnDebug("done");
